...
data_dir = '/home/SENSETIME/huanghaian/dataset/project/unet'

img_list = glob.glob(data_dir + os.sep + '*')
tra_img_name_list = list(filter(lambda f: f.find('jpg') < 0, img_list))
tra_lbl_name_list = []

for tra_img in tra_img_name_list:
    fname, name1 = os.path.split(tra_img)
    aaa = name1.split(".")
    mask_path = os.path.join(fname, name1[:-3]+'json')
